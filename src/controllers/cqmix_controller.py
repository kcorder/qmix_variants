            ...
            if exploration_mode == "ornstein_uhlenbeck":
                ...
                if t_env >= start_steps:
                    ...
                    if self.args.env_args["scenario"] in ["Humanoid-v2", "HumanoidStandup-v2"]:
                        chosen_actions = th.from_numpy(np.array([self.args.action_spaces[0].sample()
                                                                 for i in range(self.n_agents)])).unsqueeze(0).float().to(device=ep_batch.device)
                    else:
                        chosen_actions = th.from_numpy(np.array([self.args.action_spaces[i].sample()
                                                                 for i in range(self.n_agents)])).unsqueeze(0).float().to(device=ep_batch.device)
